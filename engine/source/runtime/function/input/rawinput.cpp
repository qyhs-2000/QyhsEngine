...
namespace qyhs::input::rawinput
{
	...
	struct Internal_ControllerState
	{
		...
		std::wstring name;
		qyhs::input::ControllerState state;
	};
	...
	void ParseRawInputBlock(const RAWINPUT& raw)
	{
		...
		else if (raw.header.dwType == RIM_TYPEHID)
		{
			...
			if (!internal_controller.is_xinput) // xinput enabled controller will be handled by xinput API
			{
				...
				input::ControllerState& controller = internal_controller.state;

				for (ULONG i = 0; i < numberOfButtons; i++)
				{
					int button = usage[i] - pButtonCaps->Range.UsageMin;
					controller.buttons |= 1 << (button + input::GAMEPAD_BUTTON_1 - input::GAMEPAD_RANGE_START - 1);
				}
				...
				for (USHORT i = 0; i < Caps.NumberInputValueCaps; i++)
				{
					...
					case 0x39:  // Hat Switch
						...
						case POV_UPRIGHT:
						case POV_LEFTUP:
							controller.buttons |= 1 << (input::GAMEPAD_BUTTON_UP - input::GAMEPAD_RANGE_START - 1);
							break;
						}
						switch (pov)
						{
						...
						case POV_UPRIGHT:
						case POV_RIGHTDOWN:
							controller.buttons |= 1 << (input::GAMEPAD_BUTTON_RIGHT - input::GAMEPAD_RANGE_START - 1);
							break;
						}
						switch (pov)
						{
						...
						case POV_RIGHTDOWN:
						case POV_DOWNLEFT:
							controller.buttons |= 1 << (input::GAMEPAD_BUTTON_DOWN - input::GAMEPAD_RANGE_START - 1);
							break;
						}
						switch (pov)
						{
						...
						case POV_DOWNLEFT:
						case POV_LEFTUP:
							controller.buttons |= 1 << (input::GAMEPAD_BUTTON_LEFT - input::GAMEPAD_RANGE_START - 1);
							break;
						}
					}
						break;
	...
	void Update()
	{
		...
		{
			internal_controller.state = input::ControllerState();
		}
	...
	bool GetControllerState(input::ControllerState* state, int index)
	{
	...
	void SetControllerFeedback(const input::ControllerFeedback& data, int index)
	{
		if (index < (int)controllers.size() && controllers[index].handle && !controllers[index].is_xinput)
		{
...
namespace input::rawinput
{
	...
	bool GetControllerState(input::ControllerState* state, int index) { return false; }
	void SetControllerFeedback(const input::ControllerFeedback& data, int index) {}
}
