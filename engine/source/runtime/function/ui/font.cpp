...
#include "shader/ShaderInterop_Font.h"
#include "rect_packer.h"
#include "function/render/texture_helper.h"
...
namespace qyhs::font
{
	...
	struct FontStyle
	{
		...
		void create(const std::string& new_name, const uint8_t* data, size_t size)
		{
			name = new_name;
	...
	static std::unordered_map<int32_t, Glyph> glyph_lookup;
	static std::unordered_map<int32_t, qyhs::rectpacker::Rect> rect_lookup;
	struct Bitmap
	{
		int width;
		int height;
	...
	static std::unordered_map<int32_t, Bitmap> bitmap_lookup;
	union GlyphHash
	{
		struct
		{
	...
	static std::unordered_set<uint32_t> pendingGlyphs;
	...
	static std::vector<std::unique_ptr<FontStyle>> font_styles;
	static std::mutex locker;
	static RasterizerState rasterizer_state;
	static BlendState blend_state;
	static DepthStencilState depth_stencil_states[DEPTH_TEST_MODE_COUNT];
	static Shader vertex_shader;
	static Shader pixel_shader;
	static PipelineState pso[DEPTH_TEST_MODE_COUNT];

	static thread_local std::vector<FontVertex> vertexList;
	ParseStatus ParseText(const wchar_t* text, size_t text_length, const Params& params)
	{
		...
		auto word_wrap = [&] {
			...
			if (status.last_word_begin > 0 && params.h_wrap >= 0 && status.cursor.position.x >= params.h_wrap - 1)
			{
				...
				for (size_t i = status.last_word_begin; i < status.quad_count * 4; ++i)
				{
		...
		for (size_t i = 0; i < text_length; ++i)
		{
			...
			else
			{
				...
				const size_t vertexID = size_t(status.quad_count) * 4;
	...
	ParseStatus ParseText(const char* text, size_t text_length, const Params& params)
	{
		...
		helper::stringConvert(char_temp_buffer, wchar_temp_buffer);
	...
	template<typename T>
	Cursor draw_internal(const T* text, uint32_t length, Params params, CommandList cmd)
	{
		if (length < 0)
		{
			return Cursor();
		}
		ParseStatus status = ParseText(text, length, params);
		if (status.quad_count > 0)
		{
			RHI* rhi = rhi::getRHI();
			RHI::GPUAllocation mem = rhi->allocateGPU(sizeof(FontVertex) * status.quad_count * 4, cmd);
			if (!mem.isValid())
			{
			...
			FontConstants font = {};
			font.buffer_index = rhi->getDescriptorIndex(&mem.buffer, SubresourceType::SRV);
			font.buffer_offset = (uint32_t)mem.offset;
			font.texture_index = rhi->getDescriptorIndex(&texture, SubresourceType::SRV);
			if (font.buffer_index < 0 || font.texture_index < 0)
			{
				return status.cursor;
			}
			rhi->beginEvent("Font", cmd);
			rhi->bindPipelineState(&pso[params.isDepthTestEnabled()], cmd);
			font.flags = 0;
			...
			float vertical_flip = params.custom_projection == nullptr ? 1.0f : -1.0f;
			if (params.h_align == FONT_ALIGN_CENTER)
				offset.x -= status.cursor.size.x / 2;
			else if (params.h_align == FONT_ALIGN_RIGHT)
				offset.x -= status.cursor.size.x;
			if (params.v_align == FONT_ALIGN_CENTER)
				offset.y -= status.cursor.size.y / 2 * vertical_flip;
			else if (params.v_align == FONT_ALIGN_BOTTOM)
			...
			if (params.shadowColor.getA() > 0)
			{
				...
				rhi->bindDynamicConstantBuffer(font, CBSLOT_FONT, cmd);

				rhi->drawInstanced(4, status.quad_count, 0, 0, cmd);
			}
	...
	{
		draw_internal(text, length, params, cmd);
	}

	int addFontStyle(const std::string& font_name, const uint8_t* data, size_t size, bool copy_data)
	{
		std::scoped_lock lock(locker);
		for (int i = 0; i < font_styles.size(); ++i)
		{
			const FontStyle& font_style = *font_styles[i];
			if (font_style.name.compare(font_name) == 0)
			{
				return i;
			}
		}
		font_styles.push_back(std::make_unique<FontStyle>());
		font_styles.back()->create(font_name, data, size);
		invalidateAtlas();
	}

	void loadShaders()
	{
		renderer::loadShader(ShaderStage::VERTEX_SHADER, vertex_shader, "fontVS.cso");
		renderer::loadShader(ShaderStage::PIXEL_SHADER, pixel_shader, "fontPS.cso");
	...
	void updateAtlas(float upscaling)
	{
		...
		if (!pendingGlyphs.empty())
		{
			...
			static thread_local qyhs::rectpacker::State packer;
			packer.clear();
			...
			if (packer.pack(4096))
			{
				...
				texturehelper::createTexture(texture, atlas.data(), atlasWidth, atlasHeight, Format::R8_UNORM);
				rhi::getRHI()->setName(&texture, "wi::font::texture");
			}
			else
			{
