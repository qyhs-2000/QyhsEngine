...
namespace QYHS
{
	...
	void MainCameraRenderPass::drawMesh()
	{
		...
		std::map<Material*, std::map<Mesh*, std::vector<MeshNode>>>	main_camera_pass_mesh_batch;
		...
		if (m_visible_render_meshes.p_main_camera_pass_visible_mesh_nodes)
		{
			...
			vkCmdBindPipeline(m_vulkan_rhi->getCurrentCommandBuffer(), VK_PIPELINE_BIND_POINT_GRAPHICS, m_render_pipelines[per_mesh].pipeline);
			...
			{
				Material& material = *(material_map_to_meshinstance.first);
				...
				for (auto& pair : mesh_instance)
				{
					...
					VkBuffer vertexBuffers[] = { mesh.mesh_vertex_position_buffer };
					VkBuffer indices_buffer = mesh.mesh_vertex_index_buffer;
					size_t indices_size = mesh.indices_size;
					VkDeviceSize offsets[] = { 0 };

					vkCmdBindVertexBuffers(m_vulkan_rhi->getCurrentCommandBuffer(), 0, 1, vertexBuffers, offsets);

					vkCmdBindIndexBuffer(m_vulkan_rhi->getCurrentCommandBuffer(), indices_buffer, 0, VK_INDEX_TYPE_UINT32);
					vkCmdDrawIndexed(m_vulkan_rhi->getCurrentCommandBuffer(), static_cast<uint32_t>(indices_size), 1, 0, 0, 0);
	...
	void MainCameraRenderPass::setupAttachments()
	{
		...
		for (size_t i = 0; i < main_camera_pass_attachment_count; ++i)
		{
			...
			VulkanUtil::createImage(m_vulkan_rhi->getDevice(), m_vulkan_rhi->getPhysicalDevice(), extent.width, extent.height,
			...
			m_framebuffer.attachments[main_camera_pass_color_attachment].image_view
				= VulkanUtil::createImageView(m_vulkan_rhi->getDevice(),
					m_framebuffer.attachments[main_camera_pass_color_attachment].image,
					m_framebuffer.attachments[main_camera_pass_color_attachment].format,
					1, VK_IMAGE_ASPECT_COLOR_BIT);
			//create depth attachment
			VulkanUtil::createImage(m_vulkan_rhi->getDevice(), m_vulkan_rhi->getPhysicalDevice(),
	...
	void MainCameraRenderPass::setupRenderPipelines()
	{
		...
		{
			auto vertShaderCode = Util::readFile("E://VS_Project//QyhsEngine//QyhsRuntime//shader//vert.spv");
			auto fragShaderCode = Util::readFile("E://VS_Project//QyhsEngine//QyhsRuntime//shader//frag.spv");

			VkShaderModule vertShaderModule = VulkanUtil::createShaderModule(m_vulkan_rhi->getDevice(), vertShaderCode);
			VkShaderModule fragShaderModule = VulkanUtil::createShaderModule(m_vulkan_rhi->getDevice(), fragShaderCode);
			...
			VkDescriptorSetLayout  descriptorSetLayout[1] = { m_descriptors[per_mesh].descriptor_set_layout };
			...
			if (vkCreateGraphicsPipelines(m_vulkan_rhi->getDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_render_pipelines[per_mesh].pipeline) != VK_SUCCESS) {
	...
	void MainCameraRenderPass::setupDescriptorSets()
	{
		...
		allocInfo.pSetLayouts = &m_descriptors[per_mesh].descriptor_set_layout;

		if (vkAllocateDescriptorSets(m_vulkan_rhi->getDevice(), &allocInfo, &m_descriptors[per_mesh].descriptor_set) != VK_SUCCESS) {
		...
		for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
			...
			descriptorWrites[0].dstSet = m_descriptors[per_mesh].descriptor_set;
			...
			descriptorWrites[1].dstSet = m_descriptors[per_mesh].descriptor_set;
	...
	void MainCameraRenderPass::setupDescriptorSetLayout()
	{
		m_descriptors.resize(descriptor_set_layout_type_count);
		{
			...
			if (vkCreateDescriptorSetLayout(m_vulkan_rhi->getDevice(), &layoutInfo, nullptr, &m_descriptors[per_mesh].descriptor_set_layout) != VK_SUCCESS) {
