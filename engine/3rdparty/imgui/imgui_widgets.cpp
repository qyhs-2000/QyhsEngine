...
// - PressedOnDragDropHold can generally be associated with any flags.
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    if (g.NavActivateDownId == id)
    {
        ...
        {
            // Set activate id so it can be queried by user via IsItemActive(), equivalent of holding the mouse button.
            pressed = true;
            SetActiveID(id, window);
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    if ((flags & ImGuiButtonFlags_AlignTextBaseLine) && style.FramePadding.y < window->DC.CurrLineTextBaseOffset) // Try to vertically align buttons that are smaller/have no padding so that text baseline matches (bit hacky, since it shouldn't be a flags)
...
bool ImGui::BeginComboPreview()
{
    ...
    if (window->SkipItems || !window->ClipRect.Overlaps(g.LastItemData.Rect)) // FIXME: Because we don't have a ImGuiItemStatusFlags_Visible flags to test last ItemAdd() result
        return false;
    IM_ASSERT(g.LastItemData.Rect.Min.x == preview_data->PreviewRect.Min.x && g.LastItemData.Rect.Min.y == preview_data->PreviewRect.Min.y); // Didn't call after BeginCombo/EndCombo block or forgot to pass ImGuiComboFlags_CustomPreview flags?
...
{
    // FIXME-OPT: we could pre-compute the indices to fasten this. But only 1 activate combo means the waste is limited.
...
// This is called by DragBehavior() when the widget is activate (held by mouse or being manipulated with Nav controls)
...
// Create text input in place of another activate widget (e.g. used when doing a CTRL+Click on drag/slider widgets)
...
bool ImGui::TempInputText(const ImRect& bb, ImGuiID id, const char* label, char* buf, int buf_size, ImGuiInputTextFlags flags)
{
    ...
    if (init)
    {
        // First frame we started displaying the InputText widget, we expect it to take the activate id.
...
// Note that Drag/Slider functions are only forwarding the min/max values clamping values if the ImGuiSliderFlags_AlwaysClamp flags is set!
...
{
    // Hexadecimal input provided as a convenience but the flags name is awkward. Typically you'd use InputText() to parse your own data, if you want to handle prefixes.
...
// - When activate, hold on a privately held copy of the text (and apply back to 'buf'). So changing 'buf' while the InputText is activate has no effect.
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const bool is_resizable = (flags & ImGuiInputTextFlags_CallbackResize) != 0;
    if (is_resizable)
        IM_ASSERT(callback != NULL); // Must provide a callback if you set the ImGuiInputTextFlags_CallbackResize flags!
    ...
    {
        // Support for internal ImGuiInputTextFlags_MergedItem flags, which could be redesigned as an ItemFlags if needed (with test performed in ItemAdd)
        ItemSize(total_bb, style.FramePadding.y);
    ...
    // We are only allowed to access the state if we are already the activate widget.
    ...
    // FIXME-OPT: Because our selection/cursor code currently needs the wide text we need to convert it when activate, which is not ideal :(
    ...
    if (g.ActiveId == id)
    {
        ...
        // Although we are activate we don't prevent mouse from hovering other elements unless we are interacting right now with the widget.
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        // Update render selection flags after events have been handled, so selection highlight can be displayed during the same frame.
    ...
    if (g.ActiveId == id)
    {
        ...
        if (apply_edit_back_to_user_buffer)
        {
            ...
            // Note that as soon as the input box is activate, the in-widget value gets priority over any underlying modification of the input buffer
            // FIXME: We actually always render 'buf' when calling DrawList->AddText, making the comment above incorrect.
            // FIXME-OPT: CPU waste to do this every time the widget is activate, should mark dirty state from the stb_textedit callbacks.
            if (!is_readonly)
            {
    ...
    // Release activate ID at the end of the function (so e.g. pressing Return still does a final application of the value)
    ...
    // Render text. We currently only render selection when the widget is activate or while scrolling.
    ...
    if (is_multiline)
    {
        ...
        // ...and then we need to undo the group overriding last item data, which gets a bit messy as EndGroup() tries to forward scrollbar being activate...
...
// See enum ImGuiColorEditFlags_ for available options. e.g. Only access 3 floats if ImGuiColorEditFlags_NoAlpha flags is set.
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        for (int n = 0; n < components; n++)
        {
            ...
            // FIXME: When ImGuiColorEditFlags_HDR flags is passed HS values snap in weird ways when SV values go below 0.
    ...
    // NB: The flags test is merely an optional micro-optimization, BeginDragDropTarget() does the same test.
    ...
    // When picker is being actively used, use its activate id so IsItemActive() will function on ColorEdit4().
...
// Note: ColorPicker4() only accesses 3 floats if ImGuiColorEditFlags_NoAlpha flags is set.
...
// Note: only access 3 floats if ImGuiColorEditFlags_NoAlpha flags is set.
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    // Store a flags for the current depth to tell if we will allow closing this node when navigating one of its child.
    ...
    // We set ImGuiButtonFlags_PressedOnClickRelease on OpenOnDoubleClick because we want the item to be activate on the initial MouseDown in order for drag and drop to work.
...
// CollapsingHeader returns true when opened but do not indent nor push into the ID stack (because of the ImGuiTreeNodeFlags_NoTreePushOnOpen flags).
// This is basically the same as calling TreeNodeEx(label, ImGuiTreeNodeFlags_CollapsingHeader). You can remove the _NoTreePushOnOpen flags if you want behavior closer to normal TreeNode().
...
static bool IsRootOfOpenMenuSet()
{
    ...
    // Previously this worked between popup and a first child menu because the first child menu always had the _ChildWindow flags,
    // making  hovering on parent popup possible while first child menu was focused - but this was generally a bug with other side effects.
    // Instead we don't treat Popup specifically (in order to consistently support menu features in them), maybe the first child menu of a Popup
    // doesn't have the _ChildWindow flags, and we rely on this IsRootOfOpenMenuSet() check to allow hovering between root window/popup and first chilld menu.
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    // The first menu in a hierarchies isn't so hovering doesn't get accross (otherwise e.g. resizing borders with ImGuiButtonFlags_FlattenChildren would react), but top-most BeginMenu() will bypass that limitation.
...
bool    ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags, ImGuiDockNode* dock_node)
{
    ...
    // Ensure correct ordering when toggling ImGuiTabBarFlags_Reorderable flags, or when a new tab was added while being not reorderable
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        // and we cannot wait for the next BeginTabItem() call. We cannot compute this width within TabBarAddTab() because font size depends on the activate window.
    ...
    // If we have lost the selected tab, select the next most recently activate one
...
void ImGui::TabBarAddTab(ImGuiTabBar* tab_bar, ImGuiTabItemFlags tab_flags, ImGuiWindow* window)
{
    ...
    IM_ASSERT(g.CurrentTabBar != tab_bar);  // Can't work while the tab bar is activate as our tab doesn't have an X offset yet, in theory we could/should test something like (tab_bar->CurrFrameVisible < g.FrameCount) but we'd need to solve why triggers the commented early-out assert in BeginTabBarEx() (probably dock node going from implicit to explicit in same frame)
...
void ImGui::TabBarCloseTab(ImGuiTabBar* tab_bar, ImGuiTabItem* tab)
{
    ...
    if (!(tab->Flags & ImGuiTabItemFlags_UnsavedDocument))
    {
        ...
        // However we don't run it in the case where the 'Unsaved' flags is set, so user gets a chance to fully undo the closure
        tab->WantClose = true;
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    // Transfer activate id window so the activate id is not owned by the dock host (as StartMouseMovingWindow()
    // will only do it on the drag). This allows FocusWindow() to be more conservative in how it clears activate id.
    ...
    // (We test IsItemHovered() to discard e.g. when another item is activate or drag and drop over the tab bar, which g.HoveredId ignores)
