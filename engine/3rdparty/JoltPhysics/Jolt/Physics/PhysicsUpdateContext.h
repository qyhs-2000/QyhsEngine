...
public:
	...
	struct SubStep
	{
		...
		atomic<uint32>		mIntegrateVelocityReadIdx { 0 };						///< Next activate body index to take when integrating velocities
	...
	struct Step
	{
		...
		uint32				mNumActiveBodiesAtStepStart;							///< Number of bodies that were activate at the start of the physics update step. Only these bodies will receive gravity (they are the first N in the activate body list).

		alignas(JPH_CACHE_LINE_SIZE) atomic<uint32>	mConstraintReadIdx { 0 };		///< Next constraint for determine activate constraints (moved to own cache line since its modified frequently by different jobs)
		...
		alignas(JPH_CACHE_LINE_SIZE) atomic<uint32>	mActiveBodyReadIdx { 0 };		///< Index of fist activate body that has not yet been processed by the broadphase
		...
		atomic<JobMask>		mActiveFindCollisionJobs;								///< A bitmask that indicates which jobs are still activate
		...
		JobHandleArray		mDetermineActiveConstraints;							///< Determine which constraints will be activate during this step
		JobHandleArray		mApplyGravity;											///< Update velocities of bodies with gravity
		JobHandleArray		mFindCollisions;										///< Find all collisions between activate bodies an the world
	...
	Constraint **			mActiveConstraints = nullptr;							///< Constraints that were activate at the start of the physics update step (activating bodies can activate constraints and we need a consistent snapshot). Only these constraints will be resolved.
