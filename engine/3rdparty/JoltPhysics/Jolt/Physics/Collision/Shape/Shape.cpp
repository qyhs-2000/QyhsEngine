...
void Shape::SaveWithChildren(StreamOut &inStream, ShapeToIDMap &ioShapeMap, MaterialToIDMap &ioMaterialMap) const
{
	...
	if (shape_id_iter == ioShapeMap.end())
	{
		...
		for (const PhysicsMaterial *mat : materials)
		{
			...
			{
				MaterialToIDMap::const_iterator material_asset_id = ioMaterialMap.find(mat);
				if (material_asset_id == ioMaterialMap.end())
				{
				...
				else
				{
					...
					inStream.Write(material_asset_id->second);
				}
			}
		}
	}
...
Shape::ShapeResult Shape::sRestoreWithChildren(StreamIn &inStream, IDToShapeMap &ioShapeMap, IDToMaterialMap &ioMaterialMap)
{
	...
	for (size_t i = 0; i < len; ++i)
	{
		...
		uint32 material_asset_id;
		inStream.Read(material_asset_id);

		// Check nullptr
		if (material_asset_id != ~uint32(0))
		{
			if (material_asset_id >= ioMaterialMap.size())
			{
				...
				JPH_ASSERT(material_asset_id == ioMaterialMap.size());
				ioMaterialMap.push_back(material);
			}
			else
			{
				// Existing material
				material = ioMaterialMap[material_asset_id];
			}
		}
