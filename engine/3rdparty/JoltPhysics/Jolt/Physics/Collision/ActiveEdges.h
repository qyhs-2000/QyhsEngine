...
/// An activate edge is an edge that either has no neighbouring edge or if the angle between the two connecting faces is too large.
namespace ActiveEdges
{
	/// Helper function to check if an edge is activate or not
	...
	{
		// If normals are opposite the edges are activate (the triangles are back to back)
		float cos_angle_normals = inNormal1.Dot(inNormal2);
		if (cos_angle_normals < -0.99984769515639123915701155881391f) // cos(179 degrees)
			return true;
			
		// Check if concave edge, if so we are not activate
		if (inNormal1.Cross(inNormal2).Dot(inEdgeDirection) < 0.0f)
			return false;

		// Convex edge, activate when angle bigger than threshold
	...
	/// @return Returns inNormal if an activate edge was hit, otherwise returns inTriangleNormal
	...
	{
		// Check: All of the edges are activate, we have the correct normal already. No need to call this function!
		...
		// Check: None of the edges are activate, we need to use the triangle normal
		if (inActiveEdges == 0)
			return inTriangleNormal;

		// Some edges are activate.
		// If normal is parallel to the triangle normal we don't need to check the activate edges.
		...
		{
			// Colliding with v0, edge 0 or 2 needs to be activate
			colliding_edge = 0b101;
		}
		else if (v > cOneMinusEpsilon)
		{
			// Colliding with v1, edge 0 or 1 needs to be activate
			colliding_edge = 0b011;
		}
		else if (w > cOneMinusEpsilon)
		{
			// Colliding with v2, edge 1 or 2 needs to be activate
			colliding_edge = 0b110;
		}
		else if (u < cEpsilon)
		{
			// Colliding with edge v1, v2, edge 1 needs to be activate
			colliding_edge = 0b010;
		}
		else if (v < cEpsilon)
		{
			// Colliding with edge v0, v2, edge 2 needs to be activate
			colliding_edge = 0b100;
		}
		else if (w < cEpsilon)
		{
			// Colliding with edge v0, v1, edge 0 needs to be activate
		...
		// If this edge is activate, use the provided normal instead of the triangle normal
