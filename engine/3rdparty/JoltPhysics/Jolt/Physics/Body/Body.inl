...
inline bool Body::sFindCollidingPairsCanCollide(const Body &inBody1, const Body &inBody2)
{
	...
	// Check that body 1 is activate
	uint32 body1_index_in_active_bodies = inBody1.GetIndexInActiveBodiesInternal();
	JPH_ASSERT(!inBody1.IsStatic() && body1_index_in_active_bodies != Body::cInactiveIndex, "This function assumes that Body 1 is activate");
	...
	//	- A is activate and B is not yet activate (3)
	//	- A is activate and B will become activate during this simulation step (4)
	//	- A is activate and B is activate, we require a condition that makes A, B collide and B, A not (5)
	//
	// In order to implement this we use the index in the activate body list and make use of the fact that 
	// a body not in the activate list has Body.Index = 0xffffffff which is the highest possible value for an uint32.
	//
	// Because we know that A is activate we know that A.Index != 0xffffffff:
	// (1) Because A.Index != 0xffffffff, if A.Index = B.Index then A = B, so to collide A.Index != B.Index
	// (2) A.Index != 0xffffffff, B.Index = 0xffffffff (because it's static and cannot be in the activate list), so to collide A.Index != B.Index
	// (3) A.Index != 0xffffffff, B.Index = 0xffffffff (because it's not yet activate), so to collide A.Index != B.Index
	...
	//     will be added to the end of the activate list which will make B.Index > A.Index (this holds only true when we don't deactivate 
