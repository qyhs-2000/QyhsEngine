    ...
    _In_ int       nCmdShow)
{
    ...
    editor.info_display.active = true;
    editor.info_display.watermark = true;
    editor.info_display.resolution = true;
    editor.info_display.fpsinfo = true;
    ...
    while (msg.message != WM_QUIT)
    {
        if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
            ...
            editor.run2(); // run the update - render loop (mandatory)
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
    ...
    editor.setWindow(hWnd); // assign window handle (mandatory)
